...
def revenue_risk(row: pd.Series) -> float:
    # Interpretable prototype score: amount + customer value + failure recoverability - risk.
...
def candidate_actions(row: pd.Series) -> list[Action]:
    ...
    if row.failure_reason in {"bank_timeout", "upi_declined", "auth_failed", "subscription_failed"} and row.retry_count < 2:
...
def policy_gate(row: pd.Series, action: Action) -> tuple[bool, list[str]]:
    checks = []
    ok = True
    if row.amount <= 10000:
        checks.append("amount within autonomous limit")
    else:
        checks.append("high-value transaction")
        ok = False
    if row.retry_count < 2:
        checks.append("retry budget available")
    else:
        checks.append("retry stopping rule reached")
        if action.name == "Smart retry": ok = False
    if row.risk_score < .65:
        checks.append("risk below safety threshold")
    else:
        checks.append("risk requires review")
        ok = False
    if action.name == "Human escalation": ok = False
    if not action.auto_allowed: ok = False
    return ok, checks
...
def analyze(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["auto_approved"] = out.apply(lambda r: policy_gate(r, r.actions[0])[0], axis=1)
    return out


def stable_success(transaction_id: str, probability: float) -> bool:
    # Deterministic demo outcome: repeatable between page refreshes.
    digest = hashlib.sha256(transaction_id.encode()).hexdigest()
    value = int(digest[:8], 16) / 0xFFFFFFFF
    return value < probability


def run_batch(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    results = []
    for _, row in df.iterrows():
        action = row.actions[0]
        approved, checks = policy_gate(row, action)
        executed = approved
        recovered = 0.0
        status = "blocked"
        if executed:
